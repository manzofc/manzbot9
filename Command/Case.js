...
const fake = 'Subrek YT : MANZ 08'
KirSelf = false

fakekir = fs.readFileSync('./A/image/fake.jpg')

module.exports = KirBotz = async (KirBotz, kiryt) => {
try {
...
let po = KirBotz.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "SELECT","footerText": "_*©KirBotz×*_","listType": "SELECT","sections": list}}, {})
...
const dDisplay = d > 0 ? d + (d == 1 ? " ʜᴀʀɪ, " : " ʜᴀʀɪ, ") : "";
const hDisplay = h > 0 ? h + (h == 1 ? " ᴊᴀᴍ, " : " ᴊᴀᴍ, ") : "";
const mDisplay = m > 0 ? m + (m == 1 ? " ᴍᴇɴɪᴛ, " : " ᴍᴇɴɪᴛ, ") : "";
const sDisplay = s > 0 ? s + (s == 1 ? " ᴅᴇᴛɪᴋ, " : " ᴅᴇᴛɪᴋ, ") : "";
...
return reply('「 ❗ 」ᴊᴀɴɢᴀɴ sᴘᴀᴍ ᴊᴇᴅᴀ 5 ᴅᴇᴛɪᴋ')}
...
「 *ᴅᴏᴡɴʟᴏᴀᴅ ᴍᴇɴᴜ* 」
     ...
     ${Yy+=1} ${prefix}playstore`
sendButMessage(from, titid, `ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴍᴀɴᴢ`, [
{
...
break
case 'sewabot':
gopeynya = 'https://telegra.ph/file/111c6eb02ae0fcfc2ca5a.jpg'
...
*🌹 ᴘᴀʏᴍᴇɴᴛ 🌹*
ɢᴏᴘᴀʏ : https://telegra.ph/file/111c6eb02ae0fcfc2ca5a.jpg
ᴏᴠᴏ : https://telegra.ph/file/c27ac601177ff5f131b29.jpg
ǫʀɪs ᴀʟʟ : https://telegra.ph/file/c27ac601177ff5f131b29.jpg
*Note* Biar Simple Scan Qris Gopay
Dan Jika Sudah Transfer
Silahkan Kirim Screenshot
Bukti Transfer Ke Owner
Nomor Owner Di Bawah
Selain Nomor Itu Clone

*🌹 ᴅᴀᴛᴀ ᴏᴡɴᴇʀ 🌹*
🔖 wa.me/62283830930643
🔖 ɪɢ ᴍᴀɴᴢʙᴏᴛ
🔖 ɢʜ ᴍᴀɴᴢᴏғᴄ
🔖 ʏᴛ ᴍᴀɴᴢ 08
🔖 © ᴅᴇᴠᴇʟᴏᴘᴇʀ ᴍᴀɴᴢ ぽ`
...
sendButImage(from, `${teksnya}`, `ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴍᴀɴᴢ`, buff, but)
break
case 'rules':
...
footerText: `ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴍᴀɴᴢ`,
description: `「 *🄸🄽🄵🄾 🅄🅂🄴🅁* 」
*↻ᴋᴏɴᴛᴀᴋ ᴜsᴇʀ* : @${sender.split('@')[0]} 
*↻ʙɪᴏ ᴜsᴇʀ* : ${thu.status}

「 *🄸🄽🄵🄾 🄱🄾🅃* 」
*↻ʀᴜɴᴛɪᴍᴇ ʙᴏᴛ* : ${runtime(process.uptime())}
*↻sᴘᴇᴇᴅ ʙᴏᴛ* : ${latensie.toFixed(4)} ᴅᴇᴛɪᴍ ⚡
*↻ᴍᴏᴅᴇ ʙᴏᴛ* : ${KirSelf ? '*sᴇʟғ*' : '*ᴘᴜʙʟɪᴄ*'}
*↻ᴛᴏᴛᴀʟ ɢʀᴜʙ ᴄʜᴀᴛ* : ${groups.length}
*↻ᴛᴏᴛᴀʟ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ* : ${privat.length}
*↻ᴛᴏᴛᴀʟ sᴇᴍᴜᴀ ᴄʜᴀᴛ* : ${totalChat.length}
*↻ᴡᴀᴋᴛᴜ ᴡɪʙ* : ${wib}
*↻ᴡᴀᴋᴛᴜ ᴡɪᴛ* : ${wit}
*↻ᴡᴀᴋᴛᴜ ᴡɪᴛᴀ* : ${wita}
*↻ᴘʀᴇғɪx* : *-> ${prefix} <-*`,
...
sendButloc(from,monospace(capti),'',`ʙᴀsᴇ ʙʏ *ᴍᴀɴᴢ*`,
[
...
pll = `Nih Kak`
sendButVideo(from, pll, `ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴍᴀɴᴢ`, kirmp4, [
{
...
pll = `Nih Kak`
sendButVideo(from, pll, `ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴍᴀɴᴢ`, get_result, [
{
...
sendButImage(from, `${q}`, `sᴜʙʀᴇᴋ ʏᴛ : ᴍᴀɴᴢ 08×`, sshaha, but)
break
case 'chiisaihentai':
case 'trap':
case 'blowjob':
...
sendButImage(from, `*Nih Kak*`, `*sᴜʙʀᴇᴋ ʏᴛ ᴍᴀɴᴢ 08*`, res, but)
break
case 'ppcp':
reply(mess.wait)
...
sendButImage(from, `Nih Versi Cowo Nya ^_^`, `*sᴜʙʀᴇᴋ ʏᴛ ᴍᴀɴᴢ 08*`, cewe, but)
break
case 'shadow':
case 'cup':
case 'cup1':
